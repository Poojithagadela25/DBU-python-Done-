...
# Wait until all elements with the class name 'product_pod' are present in the DOM
wait.until(EC.presence_of_all_elements_located((By.CLASS_NAME, 'product_pod')))

# Locate all elements on the page with the class name 'product_pod'
books = driver.find_elements(By.CLASS_NAME, 'product_pod')

# Loop through the first 10 book elements to extract data
for book in books[:10]:  # Limit to top 10 books
    # Extract the title of the book from the 'a' tag within the 'h3' tag
    title = book.find_element(By.TAG_NAME, 'h3').find_element(By.TAG_NAME, 'a').get_attribute('title')
    # Extract the price of the book from the element with class 'price_color'
